...
import { getValidationAddSyndicat } from "./validationSchemaAddSyndicat";
import LoadingClient from "@/components/Loading/Loading";
import { GetCategories } from "@/types/categories";
import useAddSyndicat from "@/hooks/settings/useAddSyndicat";

export default function AddSyndicat({
  setAddOpen,
  categories,
}: {
  ...
  const {
    ...
    resolver: zodResolver(getValidationAddSyndicat()),
    defaultValues: {
      name: "",
      ...
      <div className="flex items-center justify-center w-full h-full px-4 py-5 sm:p-6">
        <div className="w-full max-w-sm bg-white shadow-lg rounded-xl">
          <div className="px-4 py-5 sm:p-6">
            ...
            <form
              onSubmit={handleSubmit((data) => addSyndicatMutate(data))}
            ...
            >
              ...
              {isErrorAddSyndicat && message && (
                <p className="font-semibold text-red-500">{message}</p>
              )}
              {isPendingAddSyndicat && <LoadingClient />}
            </form>
          </div>
        </div>
      </div>
